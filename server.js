...
function pub(r){return{code:r.code,board:r.board,current:r.current,started:r.started,bagCount:r.bag.length,players:r.players.map((p,i)=>({token:p.token,name:p.name,score:p.score,rackCount:p.rack.length,index:i,connected:p.connected})),log:r.log.slice(-8)}}
function send(r){for(const p of r.players)if(p.id)io.to(p.id).emit("state",{...pub(r),yourToken:p.token,yourIndex:r.players.findIndex(x=>x.token===p.token),yourRack:p.rack})}
...
io.on("connection",s=>{
 s.on("createRoom",({name})=>{let r={code:code(),board:board(),bag:bag(),players:[],current:0,started:false,log:[]};rooms.set(r.code,r);let p={id:s.id,token:newToken(),name:clean(name),score:0,rack:[],connected:true};r.players.push(p);s.join(r.code);s.data.room=r.code;s.data.token=p.token;s.emit("joined",{code:r.code,token:p.token});send(r)});
 s.on("joinRoom",({code,name,token})=>{let r=rooms.get(String(code||"").toUpperCase().trim());if(!r)return s.emit("errorMsg","Room tidak ditemukan.");let old=token?byToken(r,token):null;if(old){old.id=s.id;old.connected=true;s.join(r.code);s.data.room=r.code;s.data.token=old.token;s.emit("joined",{code:r.code,token:old.token});r.log.push(`${old.name} tersambung kembali.`);send(r);return}if(r.started)return s.emit("errorMsg","Game sudah dimulai. Sambung lagi dari browser pemain lama.");if(r.players.length>=MAX)return s.emit("errorMsg","Room penuh.");let p={id:s.id,token:newToken(),name:clean(name),score:0,rack:[],connected:true};r.players.push(p);s.join(r.code);s.data.room=r.code;s.data.token=p.token;s.emit("joined",{code:r.code,token:p.token});send(r)});
 s.on("reconnectRoom",({code,token})=>{let r=rooms.get(String(code||"").toUpperCase().trim());if(!r)return s.emit("errorMsg","Room lama tidak ditemukan.");let p=byToken(r,token);if(!p)return s.emit("errorMsg","Pemain lama tidak ditemukan.");p.id=s.id;p.connected=true;s.join(r.code);s.data.room=r.code;s.data.token=p.token;s.emit("joined",{code:r.code,token:p.token});r.log.push(`${p.name} tersambung kembali.`);send(r)});
 s.on("requestState",()=>{let r=rooms.get(s.data.room);if(r)send(r)});
 s.on("startGame",()=>{let r=rooms.get(s.data.room);if(!r)return;let p=byToken(r,s.data.token);if(r.players[0]!==p)return s.emit("errorMsg","Hanya host bisa mulai.");if(r.players.length<2)return s.emit("errorMsg","Minimal 2 pemain.");r.started=true;r.players.forEach(p=>refill(r,p));r.log.push("Permainan dimulai.");send(r)});
 s.on("playMove",({placements,force})=>{let r=rooms.get(s.data.room);if(!r)return;let p=byToken(r,s.data.token);if(!p)return;if(r.players[r.current]!==p)return s.emit("errorMsg","Belum giliranmu.");let oldCurrent=r.current;let res=validate(r,placements,!!force);if(!res.ok)return s.emit("moveRejected",res);p.score+=res.score;p.rack=p.rack.filter(t=>!res.usedIds.includes(t.id));refill(r,p);r.board=res.temp;r.log.push(`${p.name}: ${res.words.join(", ")} (+${res.score})${force?" manual":""}`);r.current=(oldCurrent+1)%r.players.length;send(r)});
 s.on("passTurn",()=>{let r=rooms.get(s.data.room);if(!r)return;let p=byToken(r,s.data.token);if(!p)return;if(r.players[r.current]!==p)return s.emit("errorMsg","Belum giliranmu.");r.log.push(`${p.name} lewat.`);r.current=(r.current+1)%r.players.length;send(r)});
 s.on("disconnect",()=>{let r=rooms.get(s.data.room);if(!r)return;let p=byToken(r,s.data.token);if(p){p.connected=false;p.id=null;send(r)}})
});
